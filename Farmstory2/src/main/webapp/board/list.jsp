...
<section id="board" class="list">
    ...
    <div class="paging">
        ...
        <% if(groupStart > 1){ %>
        	<a href="/Farmstory2/board/list.jsp?group=<%= group %>&cate=<%= cate %>&pg=<%= groupStart - 1 %>" class="prev">이전</a>
        <% } %>
        
        <% for(int i=groupStart ; i<=groupEnd ; i++){ %>
        	<a href="/Farmstory2/board/list.jsp?group=<%= group %>&cate=<%= cate %>&pg=<%= i %>" class="num <%= (currentPage == i) ? "current":"" %>"><%= i %></a>
        <% } %>
        
        <% if(groupEnd < lastPageNum){ %>
        	<a href="/Farmstory2/board/list.jsp?group=<%= group %>&cate=<%= cate %>&pg=<%= groupEnd + 1 %>" class="next">다음</a>
        <% } %>
    </div>
    
    <!-- 글쓰기 버튼 -->
    <a href="/Farmstory2/board/write.jsp" class="btnWrite">글쓰기</a>
</section>
